...
class Player(object):
    ...
    def __init__(self, name, skill):
        assert skill >= 0 and skill <= 1, "skill should be from 0 to 1"
        self.name = name
...
class Score(object):
    def __init__(self, balls, diff, min_games, max_games, games_diff, min_sets):
        '''
...
def ball_result(player1, player2, std1 = 0.5, std2 = 0.2, boundary = 1.5):
    ...
    res = player1.get_skill() - player2.get_skill() + (random.random()-0.5)*std1
    if res < -boundary:
        return 2
    elif res > boundary:
        return 1
        ...
        if random.random() > prob_error:
            ...
            if random.random() < (player1.get_skill() + random.gauss(0, std2))/(player1.get_skill()+player2.get_skill()):
                return 1
            else:
                return 2
...
def play_game(p1, p2, balls, diff):
    ball_count1 = 0
    ball_count2 = 0
    while (ball_count1 <= balls and ball_count2 <= balls) or abs(ball_count1-ball_count2)< diff:
        res = ball_result(p1, p2)
        if res == 1:
            ball_count1 += 1
        elif res == 2:
            ball_count2 += 1
...
def play_set(p1, p2, balls, diff, min_games, max_games, games_diff):
    game_count1 = 0
    game_count2 = 0
    while (game_count1 < min_games and game_count2 < min_games) or (abs(game_count1 - game_count2) < games_diff and game_count1 < max_games and game_count2 < max_games):
        res = play_game(p1, p2, balls, diff)
        if res == 1:
            game_count1 +=1
        else:
            game_count2 +=1
...
def play_match(p1, p2, score):
    ...
    while sets1 < min_sets and sets2 < min_sets:
        res = play_set(p1, p2, score.balls, score.diff, score.min_games, score.max_games, score.games_diff)
        sets.append(res)
        if res[0] > res[1]:
            sets1 += 1
        else:
...
def sim_ball_res(p1, p2, std1, std2, boundary, num):
    res_one = 0
    res_two = 0
    res_zero = 0
    for i in range(num):
        res = ball_result(p1, p2, std1, std2, boundary)
        if res == 1:
            res_one += 1
        elif res == 2:
            res_two += 1
